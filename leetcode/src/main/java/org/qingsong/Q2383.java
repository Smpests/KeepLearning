...
public class Q2383 {
    /**
     * 思路：
     * 首先精力只会被消耗，所以训练后总的精力一定要大于energy中的总精力；
     * 而经验是会逐渐增加的，这部分要在计算过程中判断训练时间。
     * @param initialEnergy 初始精力
     * @param initialExperience 初始经验
     * @param energy 击败敌人需要的精力
     * @param experience 打败敌人获得的经验
     * @return 训练时间
     */
    public int minNumberOfHours(int initialEnergy, int initialExperience, int[] energy, int[] experience) {
        // stream().sum()比for慢
       // int totalEnergyOfEnemies = Arrays.stream(energy).sum();
        int totalEnergyOfEnemies = 0;
        for (int value : energy) {
            totalEnergyOfEnemies += value;
        }
        // 严格大于，则差值至少要大于1
        int trainingHours = Math.max(0, totalEnergyOfEnemies - initialEnergy + 1);
        for (int value : experience) {
            // 如果经验不够，则需要训练
            if (initialExperience <= value) {
                // 增加训练时长
                trainingHours += value - initialExperience + 1;
                // 训练后的经验比敌人经验多1即可
